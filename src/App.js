  ...
  render() { 
    return ( 
      <div className='App'>
        ...
        <button className='btn' onClick={this.toggle}>{this.state.show ? 'Hide':'show'}<h2 className='timer' >{this.state.timer}</h2></button>
        </div>
        <div>
        {this.state.show ?
        <div className='profile'>
            <div className='imgContainer'>
                <img className= 'img' src={this.state.info.imgSrc} />
            </div>
            <div className='infoContainer'>
                <h1>  {this.state.info.fullName}</h1>
